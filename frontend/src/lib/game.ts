...
export const setMove = (board: Board, player: Player, { row, col }: { row: number, col: number }) => {
...
const evaluate = (board: Board, nextPlayer: Player): { score: number, threats: Threat[] } => {
    let player: number[] = []
    let bot: number[] = []
    let threats: Threat[] = []
    board.forEach((row, i) => {
        let rowStr = movesToString(row)
        threats.push(...findThreats(rowStr, player, bot, row.map((_, j) => ({ row: i, col: j })), nextPlayer))
    ...
    for (let rowStart = 0; rowStart < BOARD_SIZE; ++rowStart) {
        ...
        for (let i = 0; i < BOARD_SIZE - rowStart; ++i) {
            diagonalMoves.push({ player: board[rowStart + i][i], row: rowStart + i, col: i })
        }
        if (diagonalMoves.length >= 5) {
            let diagonalStr = movesToString(diagonalMoves.map(m => m.player))
            threats.push(...findThreats(diagonalStr, player, bot, diagonalMoves, nextPlayer))
        }
    }
    ...
    for (let colStart = 1; colStart < BOARD_SIZE; ++colStart) {
        ...
        for (let i = 0; i < BOARD_SIZE - colStart; ++i) {
            diagonalMoves.push({ player: board[i][colStart + i], row: i, col: colStart + i })
        }
        if (diagonalMoves.length >= 5) {
            let diagonalStr = movesToString(diagonalMoves.map(m => m.player))
            threats.push(...findThreats(diagonalStr, player, bot, diagonalMoves, nextPlayer))
    ...
    for (let colStart = 0; colStart < BOARD_SIZE; ++colStart) {
        let diagonalMoves = []
        for (let i = 0; i < BOARD_SIZE - colStart; ++i) {
            diagonalMoves.push({ player: board[BOARD_SIZE - 1 - i][colStart + i], row: BOARD_SIZE - 1 - i, col: colStart + i })
        }
        if (diagonalMoves.length >= 5) {
            let diagonalStr = movesToString(diagonalMoves.map(m => m.player))
            threats.push(...findThreats(diagonalStr, player, bot, diagonalMoves, nextPlayer))
...
const checkPattern = (str: string, pattern: string, nextPlayer: Player, positions: Position[]) => {
    ...
    let score = (o - x) * 100
    let pos = pattern.split("").map((e, i) => e === "_" ? i : null).filter(e => e !== null)
    let idx = str.search(pattern)
    if (idx !== -1) {
        let scr = score + nextPlayerScore(nextPlayer, player)
        return {
            stop: scr === SCORE_5, threats: pos.map(p => ({
                score: scr,
...
const findThreats = (str: string, player: number[], bot: number[], positions: Position[], nextPlayer: Player) => {
    str = str.toLowerCase()
    let threats: Threat[] = []
    ...
    ["oooo_", "_oooo", "oo_oo", "ooo_o", "o_ooo",
        "xxxx_", "_xxxx", "xx_xx", "x_xxx", "xxx_x",
        "_ooo_", "_oo_o_", "_o_oo_",
        "_xxx_", "_xx_x_", "_x_xx_",
        "xooo__", "__ooox",
        "oxxx__", "__xxxo"
...
const isNearExistingMove = (board: Board, row: number, col: number) => {
    ...
    for (let dx of offsets) {
        for (let dy of offsets) {
            let new_row = row + dy
            let new_col = col + dx
            if (new_row >= 0 && new_row < BOARD_SIZE && new_col >= 0 && new_col < BOARD_SIZE && board[new_row][new_col] !== null) {
                return true
            }
        }
    }
...
const minimax = (board: Board, nextPlayer: Player, depth: number, isMaximizingPlayer: boolean, alpha: number, beta: number) => {
    ...
    if (threats.length === 0) {
        ...
        for (let row = 0; row < BOARD_SIZE; row++) {
            for (let col = 0; col < BOARD_SIZE; col++) {
                if (board[row][col] !== null) continue
                if (!isNearExistingMove(board, row, col)) continue
                neighbors.push({ score: Math.floor(Math.random() * 100), position: { row, col } })
            }
        }

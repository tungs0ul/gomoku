...
async fn get_rooms(
    ...
    let rooms = state
        .db
        .get_active_game_for_rooms(&rooms, &[RoomType::Bot, RoomType::Normal])
        .await
        .map_err(|err| {
            tracing::error!(?err);
...
async fn play(
    ...
    let (user_id, room_id) = match room_type {
        RoomType::Bot => {
        ...
        RoomType::Normal => {
            ...
            let mut room = None;
            {
                ...
                let r = state
                    .db
                    .get_available_quick_games(&rooms)
                    .await
                    .map_err(|err| {
                        tracing::error!(?err);
                        StatusCode::INTERNAL_SERVER_ERROR
            ...
            match room {
                Some(room) => {
                    let (room_id, mut game, ..) = room.to_owned();
                    if game.x.is_none() {
                        game.x = Some(user_id)
                    ...
                    let tx = &rooms
                        ...
                        .tx;
                    if let Err(error) = tx.send(GameEvent::Message {
                        msg: format!("{user_id} joined game"),
                        user: None,
                        id: Uuid::new_v4(),
                    }) {
                        tracing::error!(?error);
                ...
                None => {
                    ...
                    let mut game = Game::new(Uuid::new_v4(), Player::X);
                    game.x = Some(user_id);
                    state
                        .db
                        .create_room(&room_id, &RoomType::Normal, &game)
                        .await
                        .map_err(|err| {
                            tracing::error!(?err);
                            StatusCode::INTERNAL_SERVER_ERROR
                ...
                GameEvent::PlayAgain => {
                    ...
                    match game_type {
                        RoomType::Bot => {
                            ...
                            let _ = sender_state
                                .db
                                .update_game_status(&game.id, &GameStatus::Ended)
                                .await;
